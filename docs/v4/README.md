# gtrends-bayes v4 — frozen forecast bundle
...
```bash
tar -xzf gtrends-bayes-v4.tar.gz && cd gtrends-bayes-v4
...
| File | Purpose |
|---|---|
| `model/HY_v4.pkl` | Frozen posterior + AR backbone + preprocessing state for HY (HYG proxy) |
| `model/IG_v4.pkl` | Same for IG (LQD proxy) |
...
- **Re-fit the model.** Posteriors are frozen. v4 is for *using* the model;
